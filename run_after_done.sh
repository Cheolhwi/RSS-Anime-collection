...
# 记录Python脚本输出
echo "Python script output (rename): $output" >> /tmp/qbittorrent_log.txt

# 切换到目标目录，确保路径正确
cd /Volumes/media/anime || { echo "Failed to change directory"; exit 1; }

# 运行删除脚本并记录输出
delete_output=$(python3 unwanted.py)
echo "Python script output (delete): $delete_output" >> /tmp/qbittorrent_log.txt
